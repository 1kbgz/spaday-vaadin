...
lint-py:  ## run python linter with ruff
	python -m ruff check spaday_vaadin js/examples/build_pyodide_example.py
	python -m ruff format --check spaday_vaadin js/examples/build_pyodide_example.py
...
fix-py:  ## fix python formatting with ruff
	python -m ruff check --fix spaday_vaadin
	python -m ruff format spaday_vaadin
...
test-py:  ## run python tests
	python -m pytest -v spaday_vaadin/tests

# alias
tests-py: test-py

coverage-py:  ## run python tests and collect test coverage
	python -m pytest -v spaday_vaadin/tests --cov=spaday_vaadin --cov-report term-missing --cov-report xml
...
pyodide-example: build  ## build the standalone Pyodide example into dist/lite
	test -n "$(firstword $(wildcard dist/spaday_vaadin-*.whl))"
	...
	python js/examples/build_pyodide_example.py dist/lite "$(firstword $(wildcard dist/spaday_vaadin-*.whl))" dist/pyodide-deps
...
test-pyodide-example: pyodide-example  ## run the standalone Pyodide example in Chromium
	...
	cd js; SPADAY_VAADIN_PYODIDE_ONLY=1 pnpm exec playwright test tests/pyodide.spec.js
